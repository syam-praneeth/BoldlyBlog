...
author.post('/register', async (req, res) => {
    ...
    const existingAuthor = await UserAuthor
        .find
        ({ email: email });
    if (existingAuthor) {
        return res.status(200).send({ message: "Author already exists", payload: existingAuthor });
    }
    ...
    res.status(201).json({ message: 'Author registered successfully', payload: newAuthor });
});
